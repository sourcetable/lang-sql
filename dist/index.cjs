...
function completeKeywords(keywords, upperCase, section, mapper) {
    let completions = Object.keys(keywords).map(keyword => (Object.assign({ label: upperCase ? keyword.toUpperCase() : keyword, type: keywords[keyword] == Type ? "type" : keywords[keyword] == Keyword ? "keyword" : "variable", boost: -1 }, (section ? { section } : {}))));
    if (mapper)
        completions = mapper(completions);
...
*/
function keywordCompletionSource(dialect, upperCase = false, section, mapper) {
    return completeKeywords(dialect.dialect.words, upperCase, section, mapper);
}
/**
FIXME remove on 1.0 @internal
*/
function keywordCompletion(dialect, upperCase = false, section, mapper) {
...
function sql(config = {}) {
    ...
    return new language.LanguageSupport(lang.language, [schemaCompletion(config), keywordCompletion(lang, !!config.upperCaseKeywords, config.keywordsSection, config.keywordsMapper)]);
}
/**
